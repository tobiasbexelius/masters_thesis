...
Morphology::Morphology(int element_radius, int iterations) : element_radius(element_radius), iterations(iterations){
}
...
cv::Mat Morphology::morphology(cv::Mat image, int operation) {
	...
	cv::Mat structuring_element = cv::getStructuringElement(cv::MORPH_RECT, ele_size,
			anchor);
