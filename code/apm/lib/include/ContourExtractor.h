...
public:
	...
	void prunePeripheralContours(std::vector<std::vector<cv::Point>>& contours, const cv::Mat& image, double placement_constraint);
private:
	static bool isContourCentered(std::vector<cv::Point>& contour, double placement_constraint, int img_height, int img_width);
};
